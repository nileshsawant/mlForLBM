#!/usr/bin/env python3
"""
Easy LBM Neural Network Prediction and Visualization

This script provides an easy-to-use interface for generating neural network predictions
of LBM thermal flow and converting them to ParaView-compatible VTU format.

Reuses existing validated functions from the repository:
- Model loading from validate_neural_network_enhanced.py
- Geometry processing from predict_with_neural_network()
- VTU conversion from convert_nn_to_vtu.py

Author: Generated by reusing existing working functions
"""
...
def load_enhanced_model():
    ...
    Reused from validate_neural_network_enhanced.py
    """
...
def generate_predictions(model, geometry_3d, nu, temperature, timesteps):
    ...
    print(f"   Velocity range: [{velocity_pred.min():.6f}, {velocity_pred.max():.6f}]")
    print(f"   Temperature range: [{temperature_pred.min():.3f}, {temperature_pred.max():.3f}]")
...
Examples:
  ...
  python predict_and_visualize.py --geometry microstructure_geom_2.csv --nu 0.005 --temperature 0.035 --run-lbm
    ...
    parser.add_argument('--output-dir', type=str, default='custom_predictions',
                       help='Output directory (default: custom_predictions)')
    ...
    try:
        ...
        geometry_3d = load_geometry(args.geometry)
        ...
        if args.run_lbm:
            ...
            try:
                lbm_success, lbm_output_dir, plt_files = run_lbm_simulation(
                    args.nu, args.temperature, args.geometry, case_name, 
        ...
        if args.run_lbm and lbm_success:
            ...
            print(f"   • Compare predictions vs. ground truth in ParaView")
