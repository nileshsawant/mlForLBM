#!/usr/bin/env python3
"""
Convert Enhanced Neural Network Predictions to ParaView-compatible VTU format (modern VTK)

This script converts the .npz enhanced neural network predictions to VTU files
that ParaView can read without reader selection prompts.

Changes from baseline converter:
- Processes validation_enhanced/ directory
- Enhanced output naming and organization
- Handles both regular validation and seed6 generalization results
- All other conversion logic remains identical

Author: Generated for Enhanced LBM-ML ParaView visualization
"""
...
def process_validation_directory(validation_type, input_dir, output_dir):
    """
    Process a validation directory (enhanced or enhanced_seed6)
    """
...
def main():
    ...
    """
    print(" Enhanced Neural Network Predictions to ParaView VTU Converter")
    print("=" * 70)
    
    # Process both enhanced validation types
    conversion_tasks = [
        {
            "type": "ENHANCED (Seen Geometries)",
            "input_dir": "validation_enhanced/neural_network_predictions",
            "output_dir": "validation_enhanced/paraview_vtu"
        },
        {
            "type": "ENHANCED GENERALIZATION (Unseen Geometry)",
            "input_dir": "validation_seed6_enhanced/neural_network_predictions", 
            "output_dir": "validation_seed6_enhanced/paraview_vtu"
        }
    ]
    ...
    for task in conversion_tasks:
        success = process_validation_directory(
            task["type"], 
            task["input_dir"], 
            task["output_dir"]
        )
    ...
    if successful_conversions == 0:
        print(" No enhanced validation results found to convert!")
        print("\\n Make sure to run enhanced validation first:")
        print("   python validate_neural_network_enhanced.py")
        print("   python validate_seed6_minimal_enhanced.py")
    else:
        print(f" Successfully processed {successful_conversions}/2 validation types")
        
        # ParaView instructions
        print(f"\\n Enhanced ParaView Visualization Guide:")
        print(f"   1. Open ParaView")
        if successful_conversions >= 1:
            print(f"   2. For ENHANCED validation: Open .pvd files in validation_enhanced/paraview_vtu/")
        if successful_conversions >= 2:
            print(f"   3. For GENERALIZATION testing: Open .pvd files in validation_seed6_enhanced/paraview_vtu/")
        print(f"   4. VTU format loads automatically without reader selection")
        print(f"   5. Each .pvd file contains the full time series for one validation case")
        print(f"   6. Use ParaView's time controls to animate through timesteps")
        print(f"   7. Compare enhanced model performance vs LBM ground truth")
        
        print(f"\\n Analysis Suggestions:")
        print(f"   • Compare enhanced model predictions with baseline model results")
        print(f"   • Examine generalization performance on unseen geometry (seed6)")
        print(f"   • Analyze temporal evolution of physics fields")
        print(f"   • Validate enhanced model's improved spatial resolution")
